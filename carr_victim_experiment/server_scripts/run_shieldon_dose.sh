#!/bin/bash
# v1.7 dose-response: single shield-on-only V3-contrast run at arbitrary delta (server vC code)
# Usage: run_shieldon_dose.sh <seed> <delta> <outdir>
set -e
...
if [ "$DELTA" = "0.0" ]; then PNAME=clean; else PNAME=contrast; fi
python run_carr_victim.py --grid-model obstacle --constants N=6 \
  ...
  --poison-name "$PNAME" --poison-delta "$DELTA" --poison-scope shield-on \
