...
This analysis is supporting rather than load-bearing for the empirical
results.  The controlled study in Section~\ref{sec:study} establishes the
authority-transition failure and its paired clean/poison separation; the
machinery below characterizes the batch-local mechanism by which bounded
reward-record changes can cross a release-gate margin, and states the
premises under which the release-gate computations in that study are exact.
It does not construct the multi-batch exploit, which remains the locked
target-shaping rule evaluated in Section~\ref{sec:study}.
