...
the certificate semantics of Section~\ref{sec:gate} rather than the handoff and
reward-poisoning study evaluated in Section~\ref{sec:study}.
...
\subsection{Integrity-Protected Anchors and Learning-Aware FDI}

An integrity-protected state anchor can restore learning by shrinking the
plausible-state set.  At $\rho=0.9$, an integrity-protected sign anchor reduces the set to $[0,0.9]$ or
...
Fig.~\ref{fig:lifecycle-benchmarks} separates current-action filtering from
...
Fig.~\ref{fig:parameter-gate} shows the parameter-level frontier.  At
...
Fig.~\ref{fig:matrix-parameter-gate} repeats the comparison for the matrix
parameterization.
...
Fig.~\ref{fig:adaptive-attacker} preserves the lifecycle separation under
...
benchmark.  The official LQR is augmented with a
...
Fig.~\ref{fig:safe-control-gym} separates three effects.  Action filtering
