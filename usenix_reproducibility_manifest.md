...
> Locked 2026-08-19. Run all commands from the project root. The resident-versus-release hard gate and its prerequisite smokes are included; the benign-utility development smoke is retained as negative evidence.

## Environment

| Component | Version | Source |
|---|---|---|
| manifest_lock_date | `2026-08-19` | `usenix_direction_audit_0819.md` |
| python | `3.10.12 (main, Nov 20 2023, 15:14:05) [GCC 11.4.0]` | `python3` |
...
| V5 clean-cohort learner seeds | `2300--2319` | 20 new independent training runs |
| V5 clean-cohort evaluation seeds | `9300--9319` | 24 clean-accepted states per training run |
| V6 duration-stress learner seeds | `2400--2404` | paired clean, constrained, and unconstrained conditions |
| V6 duration-stress evaluation seeds | `9400--9404` | durations 12, 24, and 48 batches |
...
```sh
python3 generate_benchmark_artifacts.py
```

### Cartpole confirmatory learner audit

```sh
python3 safe_control_gym_reinforce_reward_poisoning_sweep.py
```

### Quadrotor confirmatory learner audit

```sh
python3 safe_control_gym_quadrotor_reinforce_reward_poisoning_sweep.py
```

### Official PPO baseline and constructed target diagnostic

```sh
python3 safe_control_gym_quadrotor_ppo_baseline.py --include-target
```

### Locked PPO B-lite checkpoints

```sh
python3 safe_control_gym_quadrotor_ppo_reward_poisoning.py --batches 24 --checkpoint-batches 16 24 --training-state-count 8 --deployment-rollouts 24 --summary-out results/safe_control_gym_quadrotor_ppo_b_lite_summary.csv --rollouts-out results/safe_control_gym_quadrotor_ppo_b_lite_rollouts.csv --traces-out results/safe_control_gym_quadrotor_ppo_b_lite_traces.csv --gate-out results/safe_control_gym_quadrotor_ppo_b_lite_gate.csv
```

### Locked benign-utility calibration and smoke

```sh
python3 safe_control_gym_quadrotor_benign_utility.py --mode all
```

### P1 certificate-coverage audit

```sh
python3 safe_control_gym_quadrotor_certificate_coverage.py
```

### P2 common-state security-availability frontier

```sh
python3 safe_control_gym_quadrotor_security_availability_frontier.py
```

### P2 table and figure derivation

```sh
python3 generate_tdsc_frontier_artifacts.py
```

### P3 cache seed 2040, fractions 00:07

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py --precompute-seed 2040 --fraction-start 0 --fraction-stop 7
```

### P3 cache seed 2040, fractions 07:14

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py --precompute-seed 2040 --fraction-start 7 --fraction-stop 14
```

### P3 cache seed 2040, fractions 14:21

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py --precompute-seed 2040 --fraction-start 14 --fraction-stop 21
```

### P3 cache seed 2041, fractions 00:07

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py --precompute-seed 2041 --fraction-start 0 --fraction-stop 7
```

### P3 cache seed 2041, fractions 07:14

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py --precompute-seed 2041 --fraction-start 7 --fraction-stop 14
```

### P3 cache seed 2041, fractions 14:21

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py --precompute-seed 2041 --fraction-start 14 --fraction-stop 21
```

### P3 cache seed 2042, fractions 00:07

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py --precompute-seed 2042 --fraction-start 0 --fraction-stop 7
```

### P3 cache seed 2042, fractions 07:14

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py --precompute-seed 2042 --fraction-start 7 --fraction-stop 14
```

### P3 cache seed 2042, fractions 14:21

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py --precompute-seed 2042 --fraction-start 14 --fraction-stop 21
```

### P3 trusted-anchor audit from verified caches

```sh
python3 safe_control_gym_quadrotor_trusted_anchor_audit.py
```

### P3 table derivation

```sh
python3 generate_tdsc_anchor_artifacts.py
```

### Core and coverage table derivation

```sh
python3 generate_tdsc_result_table.py
```

### Cartpole one-step resident-kernel smoke

```sh
python3 safe_control_gym_cartpole_release_contract_smoke.py
```

### Cartpole resident predictive authority smoke

```sh
python3 safe_control_gym_cartpole_predictive_simplex_smoke.py
```

### Retrospective gate-aware target audit

```sh
python3 safe_control_gym_cartpole_certificate_evasion_smoke.py
```

### Unclipped reward-influence geometry audit

```sh
python3 safe_control_gym_cartpole_reward_geometry_smoke.py
```

### Three-seed resident-versus-release hard gate

```sh
python3 safe_control_gym_cartpole_multiseed_release_contract.py
```

### Failed prospective exact-support development smoke

```sh
python3 safe_control_gym_cartpole_prospective_gate_aware_attack.py
```

### Burned-seed reward-direction diagnostic

```sh
python3 safe_control_gym_cartpole_reward_direction_diagnostic.py
```

### Burned-seed V2 exploratory diagnostic

```sh
python3 safe_control_gym_cartpole_gate_aware_v2_exploration.py
```

### V3 prospective fixed-target development smoke

```sh
python3 safe_control_gym_cartpole_v3_fixed_target_tanh.py
```

### V3 prospective fixed-target three-seed audit

```sh
python3 safe_control_gym_cartpole_v3_fixed_target_tanh.py --learner-seeds 2070,2071,2072 --evaluation-seeds 9070,9071,9072 --confirmatory --output-prefix results/cartpole_v3_fixed_target_multiseed
```

### V3 successful-trajectory influence replay

```sh
python3 safe_control_gym_cartpole_trajectory_influence_audit.py
```

### V3 homogenized clipped-influence audit

```sh
python3 safe_control_gym_cartpole_clipped_influence_audit.py
```

### Burned-seed homogenized gate-aware post-stop diagnostic

```sh
python3 safe_control_gym_cartpole_homogenized_gate_aware_diagnostic.py
```

### V3 monitor-horizon contract sweep

```sh
python3 safe_control_gym_cartpole_horizon_contract_sweep.py
```

### Reward-log detector calibration before new seeds

```sh
python3 safe_control_gym_reward_log_detectability.py --calibrate
```

### V4 one-shot untouched five-seed confirmation (fresh artifact only)

```sh
python3 safe_control_gym_cartpole_v4_reviewer_confirmation.py
```

### V4 clean resident paired arm

```sh
python3 safe_control_gym_cartpole_v4_clean_resident_arm.py
```

### V5 adversary-free clean cohort

```sh
python3 safe_control_gym_cartpole_v5_clean_cohort.py
```

### V5 clean-cohort statistical analysis

```sh
python3 analyze_v5_clean_cohort.py
```

### V6 detection-constrained duration stress test

```sh
python3 safe_control_gym_cartpole_v6_duration_stealth_attack.py
```

### Frozen reward-log detector evaluation

```sh
python3 safe_control_gym_reward_log_detectability.py --evaluate
```

### S1 sparse reward-permutation offline verification

```sh
python3 safe_control_gym_cartpole_sparse_reward_permutation.py --stage offline_check
```

### S1 burned-seed 2070 smoke

```sh
python3 safe_control_gym_cartpole_sparse_reward_permutation.py --stage s1_2070_smoke
```

### S2 moment-preserving burned-seed 2070 fallback

```sh
python3 safe_control_gym_cartpole_moment_preserving_reward.py --stage s2_2070_smoke
```

### Finalize stealth temporal-reward work package

```sh
python3 safe_control_gym_cartpole_moment_preserving_reward.py --stage finalize_wp
```

### U1 second learner family (A2C) confirmation

```sh
python3 safe_control_gym_cartpole_a2c_reward_poisoning.py --seeds 2200,2201,2202 --canary-seed 2200 --prefix results/cartpole_a2c_reward_poisoning_multiseed
```

### U2 joint observation-FDI + reward-log confirmation

```sh
python3 safe_control_gym_cartpole_joint_channel_poisoning.py --seeds 2210,2211,2212 --canary-seed 2210 --prefix results/cartpole_joint_channel_poisoning_multiseed
```

### U3 in-loop reward-integrity defense confirmation

```sh
python3 safe_control_gym_cartpole_inloop_defense.py --seeds 2220,2221,2222 --canary-seed 2220 --prefix results/cartpole_inloop_defense_multiseed
```

### Carr instrumented-upstream reconstruction

```sh
git clone https://github.com/stevencarrau/safe_RL_POMDPs.git carr_upstream && git -C carr_upstream checkout b01dbe8b40e2167bdc1d93dea7b43e96f1a835ee && git -C carr_upstream apply --check ../carr_victim_experiment/upstream/carr_reward_poisoning.patch && git -C carr_upstream apply ../carr_victim_experiment/upstream/carr_reward_poisoning.patch
```
...
`results/tdsc_artifact_checksums.csv` contains SHA-256 digests for 310 locked source, raw-result, generated, upstream-model, figure, and submission files. `results/tdsc_environment_versions.csv` is the machine-readable environment table.
